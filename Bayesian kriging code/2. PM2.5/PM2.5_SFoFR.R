...
consts<- list(n=dim(Xft)[1], k=dim(fmat)[2], t=dim(ffmat)[2],mn=rep(0,dim(ffmat)[2]),mn2=rep(0,dim(fmat)[2]), 
              covmat=diag(sqrt(100),dim(ffmat)[2]),covmat2=diag(sqrt(100),dim(fmat)[2]),Xf=xft,ma=rep(0,N), dists=distMat)

inits_ <- list(psi=matrix(rep(0.1,dim(fmat)[2]*dim(ffmat)[2]), nrow=dim(ffmat)[2]), sigma2=1, tau2=10, rho=0.3, zero_beta=rep(0,dim(fmat)[2]), 
...
samples_SFoFR <- nimbleMCMC(model_SFoFR, data=dat_, inits=inits_,
                                   constants = consts,
                                   monitors = c("psi","sigma2","tau2","zero_beta", "lambda", "rho"),
...
for (i in 1:BB){
  for (j in 1:K){
    lambda_mat<-lambda.sample[B-(BB-i),(N*(j-1)+1):(N*j)]
...
one<-as.matrix(rep(1, dim(xft_test)[1]))
...
for(j in 1:BB){
  ...
  m <- xft_test%*%t(psii_ma[(K*(j-1)+1):(K*j),]) + one%*%t(beta0.sample[B-(BB-j),])+  t(g)%*%Ginv%*%(t(lambda_ma[(K*(j-1)+1):(K*j),]) - xft%*%t(as.matrix(psii_ma[(K*(j-1)+1):(K*j),])))
  V <- sigma2.sample[B-(BB-j)]*(Gpred - t(g)%*%Ginv%*%g)
  
  lambda_pred<-NULL
  for(k in 1:K){
    lambda.pred <- mvrnorm(1, m[,k], V)
    lambda_pred <-cbind(lambda_pred,lambda.pred)
  } 
...
for(R in 1:16){
  all<-cbind(Y_test[,R],(lambdaa/2)[,R])
  
  matplot(pts,Y_test[,R], type="l", col="red", ylim=c(min(all), max(all)))
  lines(pts,(lambdaa/2)[,R], col="blue")
}

#### MSPE ####
sqrt(mean((lambdaa/2-Y_test)^2, na.rm=T))
...
## save the result ##
save.image("japan_all_matern_nu0.5_align.RData")
save(lambdaa, Y_test, psi_ori, psi_orisam, file="final_japan_matern_nu0.5_align.RData")
save(lambdaa, Y_test, low, up, file="final_japan_matern_nu0.5_with_lowup_align.RData")
