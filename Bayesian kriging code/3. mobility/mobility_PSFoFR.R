...
## adjacency matrix
Amat<-ad_mat_korea[ind,]
Amat_test<-ad_mat_korea[-ind,]

xft<-as.matrix(age_korea)
xft<-xft%*%ffmat
P.perp <- diag(1,dim(xft)[1]) - xft%*%solve(t(xft)%*%xft)%*%t(xft)
eig <- eigen(P.perp %*% ad_mat_korea %*% P.perp)

pBase=round((dim(m_data)[2])*0.10) ## using rank as 10% of sample size
Mbase <- Re(eig$vectors[,c(1:pBase)])
p<-ncol(Mbase)
...
consts<- list(n=dim(Xft)[1], k=dim(fmat)[2], t=dim(ffmat)[2],mn=rep(0,dim(ffmat)[2]),mn2=rep(0,dim(fmat)[2]), 
              covmat=diag(sqrt(100),dim(ffmat)[2]),covmat2=diag(sqrt(100),dim(fmat)[2]),Xf=xft,M=M,MQM=MQM,ma=rep(0,p),p=p)
inits_ <- list(psi=matrix(rep(0.1,dim(fmat)[2]*dim(ffmat)[2]), nrow=dim(ffmat)[2]),zero_beta=rep(0,dim(fmat)[2]), sigma2=10, tau2=10,  
...
BB=B #number of MCMC samples using in kriging(backward)
...
## save the result ##
save.image("mobility_inout_all_rank10per_kriging_mean_pc.RData")
save(etaa, Y_test, psi_ori,psi_orisam, file="final_mobility_inout_rank10per_kriging_mean_pc.RData")
